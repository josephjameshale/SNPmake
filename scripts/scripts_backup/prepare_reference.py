...
def find_files(input_dir):
    ...
    if fasta_fname is None or gff_fname is None:
        print(f'Error: could not find both fasta and gff files in {input_dir}')
        quit(1)
...
def process_reference(input_dir, fasta_fname, gff_fname, singularity_repeatmasker, repeatmasker_lib):
    ...
    cmd2 = ['bwa', 'index', original_fasta_fname]
    subprocess.run(cmd2)
    cmd3 = ['samtools', 'faidx', original_fasta_fname]
    subprocess.run(cmd3)
    cmd4 = ['samtools', 'dict', original_fasta_fname, '-o', os.path.join(input_dir, f'{fname}.dict')]
    subprocess.run(cmd4)
