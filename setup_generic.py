...
def generic_setup(temp_reads_dir):
    sample_names = set()
    for fname in os.listdir(temp_reads_dir):
        if fname.endswith('.fastq.gz'):
            if '_R1' in fname:
                sample_names.add(fname.split('_R1')[0])
            elif '_R2' in fname:
                sample_names.add(fname.split('_R2')[0])
    with open(os.path.join('config/samples.csv'),'w') as fh:
        _ = fh.write('sample_id\n')
        for sample in sorted(sample_names):
            _ = fh.write(f'{sample}\n')
    configlines = []
    with open(os.path.join('config/config.yaml'),'r') as fh:
        for line in fh:
            if line.startswith('input_reads:'):
                configlines.append(f'input_reads: {temp_reads_dir}\n')
            else:
